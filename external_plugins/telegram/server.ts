...
// Honor HTTPS_PROXY / HTTP_PROXY (and lowercase variants) for all outbound
// Telegram API calls and file downloads. Bun's native fetch already respects
// these env vars, but we also pass `proxy` explicitly so the behavior is
// visible in logs and so callers can override per-request if needed.
const PROXY_URL =
...
if (PROXY_URL) {
  try {
    ...
    process.stderr.write(`telegram channel: routing requests via proxy ${u.protocol}//${u.host}\n`)
  } catch {
    process.stderr.write(`telegram channel: invalid proxy URL in env: ${PROXY_URL}\n`)
  }
}

// Wrapper that injects Bun's per-request `proxy` option. Used for grammy's
// Telegram API client and the ad-hoc fetch() calls that download attachments.
const proxiedFetch: typeof fetch = PROXY_URL
...
const bot = new Bot(TOKEN, {
  client: {
    // Route grammy's Telegram API traffic through the same proxy as our
    // direct fetch() calls. Without this, long-poll getUpdates and all
    // sendMessage/getFile calls would bypass the proxy.
    ...(PROXY_URL ? { baseFetchConfig: { proxy: PROXY_URL } as Record<string, unknown> } : {}),
  },
})
let botUsername = ''
